...
function try_connect_to_db()
{
  ...
  ERRORS=$("$MYSQL_PATH" -u$MYSQL_USERNAME -p$MYSQL_PWD --connect-timeout=2 -s -e';' 2>&1)
  if [[ $? != 0 ]]
  then
    STATUS_USER_SUCCESS=false
    false
...
function set_try_root_connect_to_db
{
  ...
  while true; do
    ...
    ERRORS=$("$MYSQL_PATH" -u$MYSQL_USERNAME -p$MYSQL_PWD --connect-timeout=3 -s -N -e";" 2>&1)
    if [[ $? != 0 ]]
    then
...
function execute_sql_command()
{
  ...
  export MYSQL_PWD="$MYSQL_PASSWORD"
  MYSQL_ERROR=$("$MYSQL_PATH" -u$MYSQL_USERNAME -p$MYSQL_PWD -s -N -D "$1" -e"$2" 2>&1)
  if [[ $? != 0 ]]; then
    if [ ! -z "$3" ]; then
      echo "FAILED!"
      echo ">>> $MYSQL_ERROR";
...
function execute_sql_file()
{
  ...
  export MYSQL_PWD="$MYSQL_PASSWORD"
  ERRORS=$("$MYSQL_PATH"  -u"$MYSQL_USERNAME" -p"$MYSQL_PWD" -s -N -D "$1" < "$2" 2>&1)
  if [[ $? != 0 ]]; then
    if [[ "$showstatus" = true ]]; then
      echo "FAILED!"
      echo ">>> $ERRORS";
...
function get_current_db_version()
{
  ...
  done < <("$MYSQL_PATH" -u"$MYSQL_USERNAME" -p"$MYSQL_PWD" -s -N -e"$sql")
  IFS="$OLDIFS"
}
...
function check_dbs_accessibility()
{
  ...
  ERRORS+=($("$MYSQL_PATH" -u$MYSQL_USERNAME -p$MYSQL_PWD -D$WORLD_DB_NAME --connect-timeout=2 -s -e";" 2>&1))
  if [[ $? != 0 ]];  then
    DB_WORLDDB_VERSION="0"
    ...
    #echo "$sql"
    local result=$("$MYSQL_PATH" -u$MYSQL_USERNAME -p$MYSQL_PWD -D$WORLD_DB_NAME --connect-timeout=2 -sNe"$sql" 2>&1)
    if [[ $? != 0 ]]; then
      DB_CONTENT_RELEASE_VERSION=""
    else
      # todo this may fail, we need to uniformise all db title with some title schema
  ...
  ERRORS+=($("$MYSQL_PATH" -u$MYSQL_USERNAME -p$MYSQL_PWD -D$REALM_DB_NAME --connect-timeout=2 -s -e";" 2>&1))
  if [[ $? != 0 ]]; then
    DB_REALMDB_VERSION="0"
  ...
  ERRORS+=($("$MYSQL_PATH"  -u$MYSQL_USERNAME -p$MYSQL_PWD -D$CHAR_DB_NAME --connect-timeout=2 -s -e";" 2>&1))
  if [[ $? != 0 ]]
  then
    DB_CHARDB_VERSION="0"
  ...
  ERRORS+=($("$MYSQL_PATH" -u$MYSQL_USERNAME -p$MYSQL_PWD -D$LOGS_DB_NAME --connect-timeout=2 -s -e";" 2>&1))
  if [[ $? != 0 ]]
  then
    DB_LOGSDB_VERSION="0"
...
function create_db_user_and_set_privileges()
{
  ...
  do
    ERRORS=$("$MYSQL_PATH" -u"$MYSQL_USERNAME" -p"$MYSQL_PWD" -s -N -e"$sql" 2>&1)
    if [[ $? != 0 ]]; then
      echo "FAILED!"
      echo ">>> $ERRORS"
      false
...
function print_realm_list()
{
  ...
  printf "%0.1s" "="{1..80};printf "\n"
  export MYSQL_PWD="$MYSQL_PASSWORD"
  local result=$("$MYSQL_PATH" -u"$MYSQL_USERNAME" -p"$MYSQL_PWD" -s -N -D"$REALM_DB_NAME" -e"$SQL_QUERY_REALM_LIST" 2>&1)
  if [[ $? != 0 ]]; then
...
function backup_restore_file()
{
  ...
  export MYSQL_PWD="$MYSQL_PASSWORD"
  ERRORS=$("$MYSQL_PATH" -u"$MYSQL_USERNAME" -p"$MYSQL_PWD" -D"$dbname" -sN < "$filename" 2>&1)
  if [[ $? != 0 ]]; then
    echo "FAILED!"
    echo ">>> $ERRORS"
    false
